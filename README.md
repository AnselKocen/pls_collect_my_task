...
最简单，不需要 clone 代码，双击就能用（但 AI 功能仍需要 Claude Code CLI，见下方配置说明）。
...
3. 把「记忆手帐」图标**拖到**旁边的「应用程序」文件夹上松手（这一步就是"安装"）
4. 之后从启动台或 Spotlight 搜「记忆手帐」就能启动

#### 首次打开遇到"来自身份不明开发者"

这个 app 没有 Apple 开发者签名，所以 macOS 首次启动会拦截并提示：

> "记忆手帐" 已损坏，无法打开
> 或
> 无法打开"记忆手帐"，因为它来自身份不明的开发者

**这是正常现象**，不是 app 真的坏了。解决方法：

1. 打开 **系统设置 → 隐私与安全性**
2. 往下滚到 **「安全性」** 区域
3. 会看到一行「"记忆手帐" 已被阻止使用，因为它来自身份不明的开发者」
4. 点右边的 **"仍要打开"** 按钮（macOS 14/15 可能会要求输入密码或 Touch ID 确认）
5. 再次双击启动「记忆手帐」即可正常打开
...
4. （可选）打开「**在锁屏时显示通知**」「**在通知中心显示**」等开关

改完之后，定时提醒就会一直显示在屏幕右上角，不会自动消失。
...
先在终端/命令行 `cd` 到你想放项目的位置（桌面、某个工作目录、任意盘符都行）。
...
> ⚠️ 如果你用的是 **PowerShell**（Windows 11 默认终端），直接输入 `start.bat` 会报"无法识别"。PowerShell 不允许从当前目录直接执行脚本，需要在前面加 `.\`：
>
> ```powershell
> .\start.bat     # 启动
> .\stop.bat      # 停止
> ```
>
> 传统的「命令提示符 (cmd)」不需要 `.\`，直接 `start.bat` 即可。

启动后浏览器会自动打开 http://localhost:3013

#### 日常使用（启动 / 停止）

**第一次成功启动后，以后就不需要再 clone 或 `npm install` 了。** 每次开机想用的时候，只要重新执行一下启动命令就行：
...
> 启动脚本会检测到 `node_modules` 已经装过，跳过依赖安装，直接启动服务，通常 1-2 秒就能就绪。
>
...
```cmd
cd %USERPROFILE%\Desktop\pls_collect_my_task   :: 进入项目目录
stop.bat
```
